...
###########################
##commenges-andersen score test (to test homogeneity of failure times)
# library(frailtyEM)
...
# AUCobj<-AUCtime(formula.cox="Surv(fail_time,status)~ Y + X + cluster(cluster)",formula.mark="Y~X",
#                 dati=exdata, status=exdata$status,marker.distr="NB",cov.names="X",c.values=1)
# #estimated AUC
# plot(AUCobj$ftimes, AUCobj$AUC,type="l")
# #ROC curve at time=median of failure times
# ind<-which.min(AUCobj$ftimes<=median(AUCobj$ftimes))
# plot(AUCobj$FPR[ind,],AUCobj$TPR[ind,],type="l",lty=3,
#      ylab="TPR",xlab="FPR",main="covariate-specific ROC(t=t*)")
# abline(0,1)
# legend(0.7,0.1,c(paste("AUC(t*)=",round(AUCobj$AUC[ind],digits=3),"")),lty=3)
                   ...
                   lowerP.bmk= 10e-6 ,cov.names=NULL, c.values=NULL )
{
  ...
  num<-do.call(cbind,lapply(obsvalue,FUN=function(i){
                                         k<-min(which(value>=i))
                                         numT<-(1-surv[,k:dimv])%*%P[k:dimv]
                                         numF<-surv[,k:dimv]%*%P[k:dimv]
                                         rbind(numT,numF)}))
